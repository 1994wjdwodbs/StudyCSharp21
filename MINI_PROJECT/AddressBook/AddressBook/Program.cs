...
namespace AddressBook_AND_ASCII_ART
{
    using System;
    using System.Collections.Generic;
    using System.IO;
    ...
    class AddressBook
    {
        ...
        public void Search()
        {
            ...
            while (true)
            {
                ...
                for (int i = 0; i < list.Count; i++)
                {
                    ...
                    {
                        case "1":
                            if (list[i].Item1.Contains(search))
                            {
                                int j, findIdx = list[i].Item1.IndexOf(search);

                                Console.Write("이름 : ");
                                for (j = 0; j < findIdx; j++)
                                {
                                    Console.Write(list[i].Item1[j]);
                                }
                                Console.ForegroundColor = ConsoleColor.White;
                                Console.BackgroundColor = ConsoleColor.Blue;
                                for (j = findIdx; j < findIdx + search.Length; j++)
                                {
                                    Console.Write(list[i].Item1[j]);
                                }
                                Console.ForegroundColor = ConsoleColor.White;
                                Console.BackgroundColor = ConsoleColor.Black;
                                for (j = findIdx + search.Length; j < list[i].Item1.Length; j++)
                                {
                                    Console.Write(list[i].Item1[j]);
                                }
                                Console.WriteLine("");

                                Console.WriteLine($"전화 : {list[i].Item2}");
                                Console.WriteLine($"주소 : {list[i].Item3}");
                                Console.WriteLine("------------------------");
                                count++;
                            }
                            break;
                        case "2":
                            if (list[i].Item2.Contains(search))
                            {
                                int j, findIdx = list[i].Item2.IndexOf(search);

                                Console.WriteLine($"이름 : {list[i].Item1}");
                                Console.Write("전화 : ");
                                for (j = 0; j < findIdx; j++)
                                {
                                    Console.Write(list[i].Item2[j]);
                                }
                                Console.ForegroundColor = ConsoleColor.White;
                                Console.BackgroundColor = ConsoleColor.Blue;
                                for (j = findIdx; j < findIdx + search.Length; j++)
                                ...
                                Console.ForegroundColor = ConsoleColor.White;
                                Console.BackgroundColor = ConsoleColor.Black;
                                for (j = findIdx + search.Length; j < list[i].Item2.Length; j++)
                                {
                                    Console.Write(list[i].Item2[j]);
                                }
                                Console.WriteLine("");
                            ...
                            break;
                        case "3":
                            if (list[i].Item3.Contains(search))
                            {
                                int j, findIdx = list[i].Item3.IndexOf(search);
                                ...
                                Console.ForegroundColor = ConsoleColor.White;
                                Console.BackgroundColor = ConsoleColor.Black;
                                for (j = findIdx + search.Length; j < list[i].Item3.Length; j++)
                                {
                                    Console.Write(list[i].Item3[j]);
                                }
                                Console.WriteLine("");
        ...
        * * * */
        ...
        public void SaveBookFile()
        {
            ...
            {
                Console.ForegroundColor = ConsoleColor.Red;
                Console.WriteLine("[기록할 데이터가 존재하지 않습니다.]");
                Console.ForegroundColor = ConsoleColor.White;
                System.Threading.Thread.Sleep(1000);
                return;
            }
            ...
            if (File.Exists(savePath))
            {
                using (StreamWriter outputFile = new StreamWriter(savePath, true))
                {
                    ...
                    // outputFile.Close();
                    Console.ForegroundColor = ConsoleColor.Red;
                    Console.WriteLine("[기록 완료(Append).]");
                    Console.ForegroundColor = ConsoleColor.White;
                }
            }
            // 파일이 존재하지 않으면 새로쓰기
